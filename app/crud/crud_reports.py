...
from app.models.models import MonthlyReport, MealServing, Meal, Ingredient, MealIngredient, Alert, AlertType
...
class CRUDMonthlyReport(CRUDBase[MonthlyReport, MonthlyReportCreate, MonthlyReportUpdate]):
    def get_by_month_year(self, db: Session, *, month: int, year: int) -> Optional[MonthlyReport]:
    ...
    def create_or_update_monthly_report(
        ...
        total_portions_served = db.query(func.sum(MealServing.portions)).filter(
        ...
        total_portions_possible = sum(meal["available_portions"] for meal in meals_portions)
        
        # Calculate difference percentage
        difference_percentage = 0.0
        if total_portions_possible > 0:
            difference_percentage = ((total_portions_possible - total_portions_served) / total_portions_possible) * 100
    ...
    def get_ingredient_usage_data(
        ...
        if not ingredient:
            return {"ingredient_id": ingredient_id, "ingredient_name": "Unknown", "usage_data": [], "delivery_data": []}
        ...
        meal_ingredients = db.query(MealIngredient).filter(
        ...
        deliveries = db.query(IngredientDelivery).filter(
    ...
    def get_monthly_report_data(
        ...
        for ingredient in ingredients:
            ingredient_data = self.get_ingredient_usage_data(
                db, ingredient_id=ingredient.id, start_date=start_date, end_date=end_date
            )
            
            # Calculate total usage for this ingredient
            total_usage = sum(day["usage"] for day in ingredient_data["usage_data"])
            total_delivery = sum(delivery["quantity"] for delivery in ingredient_data["delivery_data"])
