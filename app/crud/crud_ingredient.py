...
from app.schemas.ingredient import IngredientCreate, IngredientUpdate, IngredientDeliveryCreate, IngredientDeliveryUpdate
...
class CRUDIngredient(CRUDBase[Ingredient, IngredientCreate, IngredientUpdate]):
    ...
    def update_quantity(self, db: Session, *, db_obj: Ingredient, quantity_change: float) -> Ingredient:
        """
    ...
    def check_low_stock(self, db: Session) -> List[Ingredient]:
        ...
        """
        return db.query(Ingredient).filter(Ingredient.quantity < Ingredient.min_quantity).all()


class CRUDIngredientDelivery(CRUDBase[IngredientDelivery, IngredientDeliveryCreate, IngredientDeliveryUpdate]):
