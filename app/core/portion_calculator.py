...
def calculate_available_portions(db: Session, meal_id: int) -> Dict[str, Any]:
    ...
    for meal_ingredient in meal_ingredients:
        ingredient = db.query(Ingredient).filter(
            Ingredient.id == meal_ingredient.ingredient_id
        ).first()
    ...
    max_available_portions = ingredient_portions[0]["max_portions"] if ingredient_portions else 0
...
def check_ingredient_impact(db: Session, ingredient_id: int) -> List[Dict[str, Any]]:
    ...
    for meal_ingredient in meal_ingredients:
        ...
        max_portions_from_ingredient = math.floor(ingredient.quantity / meal_ingredient.quantity) if meal_ingredient.quantity > 0 else 0
        
        # Check if this ingredient is limiting the meal
        is_limiting = max_portions_from_ingredient <= current_portions["available_portions"]
    ...
    result.sort(key=lambda x: (not x["is_limiting_ingredient"], x["current_available_portions"]))
