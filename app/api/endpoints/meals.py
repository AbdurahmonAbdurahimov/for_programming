...
def create_meal(
    *,
    ...
    for ingredient_item in meal_in.ingredients:
        ingredient = crud_ingredient.ingredient.get(db, id=ingredient_item.ingredient_id)
        if not ingredient:
            raise HTTPException(
                status_code=404,
    ...
    meal = crud_meal.meal.create_with_ingredients(db, obj_in=meal_in, user_id=current_user.id)
    return meal
