...
from app.schemas.reports import MonthlyReport, MonthlyReportData, IngredientUsageData, MealServingData, Alert
...
def get_monthly_report(
    *,
    ...
    if not report:
        ...
        report = crud_reports.monthly_report.create_or_update_monthly_report(db, month=month, year=year)
    return report
...
def get_monthly_report_detailed(
    *,
    ...
    """
    report_data = crud_reports.monthly_report.get_monthly_report_data(db, month=month, year=year)
    return report_data
...
def get_alerts(
    ...
    """
    if unread_only:
        alerts = db.query(models.Alert).filter(models.Alert.is_read == False).offset(skip).limit(limit).all()
    else:
